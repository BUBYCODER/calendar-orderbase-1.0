...
<script>
        ...
        card.innerHTML = `
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:10px; margin-bottom:12px;">
                <div>
                    ...
                    <select name="product_${pId}" id="product_${pId}" onchange="onProductSelectChange(${pId}, this.value)" required>
                        <option value="">Выберите изделие...</option>
                        ${PRODUCTS_LIST.map(p => `<option value="${p}">${p}</option>`).join('')}
                    </select>
                    <input type="text" name="custom_product_${pId}" id="custom_product_${pId}" placeholder="Свое изделие..." style="display:none; margin-top:4px;">
                </div>

                <div>
                    <label class="field-title" style="display:block; margin-bottom:4px;">Лекала / Фит *</label>
                    <select name="pattern_${pId}" id="pattern_${pId}" onchange="onPatternSelectChange(${pId}, this.value)">
                        <option value="">Сначала выберите изделие</option>
                    </select>
                    <input type="text" name="custom_pattern_${pId}" id="custom_pattern_${pId}" placeholder="Свое лекало..." style="display:none; margin-top:4px;">
                </div>

                <div>
                    <label class="field-title" style="display:block; margin-bottom:4px;">Полотно *</label>
                    <select name="fabric_${pId}" id="fabric_${pId}" onchange="onFabricSelectChange(${pId}, this.value)">
                        <option value="">Выберите полотно...</option>
                        ${FABRICS_LIST.map(f => `<option value="${f}">${f}</option>`).join('')}
                    </select>
                    <input type="text" name="custom_fabric_${pId}" id="custom_fabric_${pId}" placeholder="Свое полотно..." style="display:none; margin-top:4px;">
                </div>

                <div>
                    <label class="field-title" style="display:block; margin-bottom:4px;">Плотность</label>
                    <select name="density_${pId}" id="density_${pId}" onchange="onDensitySelectChange(${pId}, this.value)">
                        <option value="">Сначала полотно</option>
                    </select>
                    <input type="text" name="custom_density_${pId}" id="custom_density_${pId}" placeholder="Плотность..." style="display:none; margin-top:4px;">
                </div>

                <div>
                    <label class="field-title" style="display:block; margin-bottom:4px;">Цвет полотна</label>
                    <select name="color_${pId}" id="color_${pId}" onchange="onColorSelectChange(${pId}, this.value)">
                        ${COLORS_LIST.map(c => `<option value="${c}">${c}</option>`).join('')}
                    </select>
                    <input type="text" name="custom_color_${pId}" id="custom_color_${pId}" placeholder="Свой цвет..." style="display:none; margin-top:4px;">
                </div>
            </div>

            <!-- Пункт 8 ТЗ: Размерная сетка (9 размеров) -->
            ...
            <!-- Пункт 9 ТЗ: кнопка нанесения левее -->
            <div style="margin-bottom:12px;">
            ...
            <!-- Пункт 10 ТЗ: Файлы к позиции -->
            <div>
                <label class="field-title" style="display:block; margin-bottom:4px;">Макеты / файлы позиции:</label>
                <input type="file" name="pos_files_${pId}" multiple accept="image/*,.pdf,.ai,.psd" style="padding:5px; font-size:12px; width:100%;">
            </div>
        `;
    ...
    // Живое автосохранение черновика и восстановление
