...
def edit_order_view(order_id):
    ...
    if request.method == 'POST':
        try:
            order_number = safe_int(request.form.get('order_number'), default=order.get('order_number') or order_id)
            ...
            creation_date = request.form.get('creation_date') or date.today().isoformat()
            amount = safe_float(request.form.get('amount'))
            order_type = request.form.get('order_type', order.get('order_type', 'batch'))
            contact = request.form.get('contact', '').strip()
            comment = request.form.get('comment', '').strip()
            o_start = request.form.get('order_start_date') or order.get('start_date') or date.today().isoformat()
            ...
            if not new_stages:
                if order_type == 'sample':
                    new_stages = [{'type': 'Образец', 'start': o_start, 'start_q': o_start_q, 'end': o_start, 'end_q': o_start_q}]
                else:
                    flash('Добавьте хотя бы один этап производства', 'error')
                    return redirect(url_for('orders.edit_order_view', order_id=order_id))
            ...
            o_end = max_stage['end']
            o_end_q = safe_int(max_stage['end_q'], default=4)
            quantity = order.get('quantity', 0)

            database.update_order_full(
                order_id, name, client_name, contact, comment, order_type, quantity,
                o_start, o_start_q, o_end, o_end_q, new_stages,
                order_number=order_number, client_id=client_id,
                contact_type=order.get('contact_type', 'Телефон'),
                amount=amount, created_at=creation_date, positions=None
            )

            try:
                upload_folder = current_app.config.get('UPLOAD_FOLDER', 'app/static/uploads')
                os.makedirs(upload_folder, exist_ok=True)
                new_files = request.files.getlist('order_files')
            ...
            flash(f'Заказ #{order_number} успешно обновлен', 'success')
            return redirect(url_for('orders.order_detail', order_id=order_id))
    ...
    clients = dict_model.get_all_clients()
    return render_template('edit_order.html', order=order, stages=stages, files=files, clients=clients)

@orders_bp.route('/delete_file/<int:file_id>', methods=['POST'])
def delete_file_view(file_id):
    order_id = request.form.get('order_id')
    file_path = database.delete_order_file(file_id)
    if file_path:
        try:
